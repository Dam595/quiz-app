      ...
      <div className="search-search-bar">
        ...
        <input
          ...
          style={{ background:'transparent', border:'none', outline:'none', flex:1, fontSize:14,
            fontFamily:'Be Vietnam Pro', color:'var(--text-dark)' }}
        />
        {query && <i className="ti ti-x" onClick={() => setQuery('')} style={{ cursor:'pointer', color:'var(--text-muted)' }} />}
      </div>
        ...
        {results.length === 0 ? (
          ...
          results.map(q => (
            <div key={q.id} className="search-result-card" onClick={() => onSelectQuiz(q)}>
              <div className="search-result-icon" style={{ background: subjectColor[q.subject] || subjectColor.default }}>
                <i className={`ti ${subjectIcon[q.subject] || subjectIcon.default}`} />
              </div>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:13, fontWeight:700, color:'var(--text-mid)', marginBottom:3 }}>{q.title}</div>
